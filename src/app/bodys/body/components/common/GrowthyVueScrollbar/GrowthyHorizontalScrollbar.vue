...
<template>
  <div>
    <div
      ...
      <div v-if="!noUseHorizontalBar"
        :class="'scrollbar' + ( dragging || draggingFromParent ? '' : ' vue-scrollbar-transition') + (classesHorizontalBar ? ` ${classesHorizontalBar}`: '')"
        ref="scrollbar"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onDrag (e) {
        if (this.dragging) {
          ...
          if (this.onDragHorizontal) {
            this.onDragHorizontal(next, 'horizontal')
          }
    ...
    beforeDestroy () {
      ...
      document.removeEventListener('touchmov', this.onDrag)
