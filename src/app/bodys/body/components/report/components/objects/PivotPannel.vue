  ...
  methods: {
    ...
    setCommonFilterAndSetting (commonCondition) {
      ...
      if(this._isUpdateGrid) {
        filter = this.pivotDataObj.filter
      }else {
        const deletedCommonFilterKeys = _.difference(Object.keys(this.prevCommonFilter), Object.keys(commonCondition.filter))
        filter = {...this.pivotDataObj.filter, ...commonCondition.filter}
        filter = _.omit(filter, deletedCommonFilterKeys)
      }
      this.prevCommonFilter = commonCondition.filter
    ...
    updateObject (commonCondition) {
      ...
      if (condition && !_.isEmpty(condition)) {
        ...
        if (condition.startDate && condition.endDate && !this._isUpdateGrid) {
            originDates = this.setOriginDates(true, condition)
          }
      } else {
        filters = this.pivotDataObj.filter
      }
      ...
      Promise.all([this.$store.dispatch(types.UPDATE_PIVOT_DATA, {isPreview: isPreview, reqBody: pivotDataObj})]).then(res => {
        ...
        this.pivotDataObj.filter = pivotDataObj.filter
